...
- **No auto-derive of reverse vsRelated refs (P6-123).** When the bidir lint flags a missing reverse-ref, do not generate a stub entry like `difference: 'auto-derived'` or copy the source-side prose verbatim. Each reverse-ref must be authored from the target's perspective — a one-line "see X" or `'auto-derived'` placeholder passes the lint syntactically but the canon loses value (the whole point of the section is the per-direction decision-prose, not a graph). Lint failure is the canonical signal that the target component needs the author to think about why a designer would pick the target *over* the source. Generation tools that fabricate the prose are rejected; manual authoring stays canonical.
