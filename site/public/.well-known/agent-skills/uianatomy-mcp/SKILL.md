...
| `get_component_section` | `id: string`, `sections: string[]` | One or more named sections in a single round-trip — the bandwidth-friendly slice. Valid sections: `anatomy`, `axes`, `mismatches`, `mistakes`, `frameworkMap`, `tokens`, `motion`, `responsive`, `transitions`, `events`, `whenToUse`, `a11yAcceptance`, `propertyMap`, `formIntegration`, `i18n`, `performance`, `sources`, `rules`. Result is an object keyed by the requested names; every section except `anatomy`/`axes`/`mismatches`/`mistakes`/`frameworkMap`/`tokens` is `null` when the component declares none; `tokens` returns only token-bearing slots (`[{ slotId, tokens }]`). `rules` (P6-201 Step 2 / ADR-039) is the component-only slice — use `get_contracts` instead when the target might be a pattern. |
...
| `get_contracts` | `id: string` | Return the hard-binding rules and vocabulary drift for a component **or** a pattern (single-tool dispatch by id). Returns `{ id, kind: "component" \| "pattern", rules, contracts }` (P6-201 Step 2 / ADR-039 — `rules` was promoted out of the former `contracts.nonNegotiable`). Each rule has `id`, `statement`, `source: 'apg' \| 'wcag' \| 'html-spec' \| 'platform' \| 'canon'`, optional `sourceRef`, and a `consequence` describing what breaks on violation; `mistakes[].ruleId` / `mismatches[].ruleId` on the full record reference these ids. `contracts.vocabularyDrift[]` is per-system attributed naming (Material 3 → Snackbar, Atlassian → Flag, …) with optional notes. Returns `{ rules: null }` / `{ contracts: null }` independently when the entity declares neither; errors on unknown id. |
...
| `list_implementations` | `componentId?: string`, `libraryId?: string`, `limit?: number` | Every Phase-2 library audit (one row per library/component pair) — `libraryId`, `componentId`, `componentName`, `divergenceCount`, `lastReviewed`. Sorted by `libraryId` then `componentId`. Optional filters narrow to one component or one library; omit both for the full roster. Coverage grows over time — call it to see the current roster rather than assuming. Use `get_implementations({ componentId })` for the full audit records. |
...
- `get_component_section({ id, sections: [...] })` — one or more sections in a single call. Valid sections: `anatomy`, `axes`, `mismatches`, `mistakes`, `frameworkMap`, `tokens`, `motion`, `responsive`, `transitions`, `events`, `whenToUse`, `a11yAcceptance`, `propertyMap`, `formIntegration`, `i18n`, `performance`, `sources`, `rules`.
...
Beyond the canon, Phase-2 library audits (Radix, Headless UI, React Aria, Angular CDK, Vaul — across a growing set of components) each document their **divergence** from the canonical anatomy as `omitted` / `renamed` / `extended` / `reshaped` entries. Call `list_implementations` for the current roster.
