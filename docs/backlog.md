...
- [ ] **P6-189 combobox (`~10`) vs. select (`~25`) Crossover-Schwelle uneins** — beide Components beschreiben denselben Select→Combobox-Kipppunkt mit unterschiedlichen Zahlen (aufgefallen bei P6-182-Fix). combobox: `whenToUse.use`/`vsRelated.select`/`vsRelated.checkbox` sagen `~10`. select: `whenToUse.use`/`vsRelated.combobox`/`performance.switchToCombobox`/`mistakes[select-too-many-options-no-search]` sagen `~25` (7 Vorkommen total). Eine Zahl als canonical wählen, andere Datei angleichen. S. Dateien: `content/components/{combobox,select}.yaml`.
