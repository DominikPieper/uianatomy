...
- [ ] **P6-163 Versioning-Subsystem (ADR-023) parken** — `since`/`changelog`/`deprecated` auf Component/Slot/Prop/Variant + `axes.variantDeprecations` + MCP-Tool `get_changelog` + `VersionBadges`/`ChangelogSection`-Render: end-to-end verdrahtet, **0/41 Adoption**. `get_changelog` gibt für alle 41 null zurück und bläht die Tool-Surface auf. Entscheidung: ADR-023 als „Accepted (dormant)" markieren + Live-Surface (Tool + Render) entfernen, Schema-Felder optional belassen; re-add wenn erstes echtes Rename landet. Von Schema- + MCP-Reviewer unabhängig geflaggt. Dateien: `mcp-server/src/server.ts`, `site/src/components/**`, `docs/adr/023-versioning.md`.
