...
- [ ] **P6-168b switch/radio-group Implementations (Batch 2)** — die anderen zwei Null-Coverage-Form-Primitives aus dem Review. switch × {radix, react-aria, headlessui}, radio-group × {radix, react-aria, headlessui (RadioGroup)}. Gleicher `library-audit-runner`-Flow wie Batch 1. Datei: `implementations/{radix,react-aria,headlessui}/{switch,radio-group}.yaml`.
- [ ] **P6-174 Canon-Gaps aus P6-168-Audit** — die menu/popover/tooltip-Audits flaggten echte Canon-Verbesserungen (keine Bugs): (a) `menu` hat keinen `group-label`-Slot (Radix/HeadlessUI `Label`/`MenuHeading` haben kein Canon-Pendant); (b) `tooltip.axes.states` modelliert nicht delayed-open vs instant-open (Radix unterscheidet sie für Exit-Animation); WCAG 1.4.13 hoverable-content-grace fehlt in `responsive`; (c) `popover` Canon kennt kein separates Anchor-Element (Radix `Popover.Anchor`); React-Aria `placement` (22-Wert-Compound) vs Canon `side`+`align` — als vocabularyDrift festhalten; (d) Canon-`events`-Payloads (`{itemId, originalEvent}`) nutzt keine reale Lib verbatim. Pro Item entscheiden: Canon erweitern oder als vocabularyDrift dokumentieren. Datei: `content/components/{menu,popover,tooltip}.yaml`.
